...
def train_weak(cfg, hwf, USE_CACHED_DATASET, device, data_dict, encode_direction_fn, encode_position_fn,
    ...
    for epoch in pbar:
        ...
        weak_train_log(cfg, device, epoch + 1, stage, loss, coarse_loss, fine_loss, psnr, lr_new, pbar, rgb_coarse, rgb_fine, grad_direction_coarse, grad_direction_fine)
        ...
        if epoch % cfg.experiment.validate_every == 0: # or epoch == cfg.experiment.weak_train_iters - 1:
            ...
            validate(cfg, hwf, USE_CACHED_DATASET, device, data_dict, encode_direction_fn, encode_position_fn, stage, epoch, net_ensemble_coarse, net_ensemble_fine, datafile_idx=1)
...
def weak_train_log(cfg, device, epoch, stage, loss, coarse_loss, fine_loss, psnr, lr, pbar, rgb_coarse, rgb_fine, target_ray_values_coarse, target_ray_values_fine):
    ...
    if epoch % 500 == 0:  # save grid of predictions
        ...
        for t in tensors:
            ...
            t = torch.cat((t[..., :3], trail))
            imgs.append(cv2.cvtColor(t.view(w, w, 3).detach().cpu().numpy() * 255, cv2.COLOR_BGR2RGB))
...
def validate(cfg, hwf, USE_CACHED_DATASET, device, data_dict, encode_direction_fn, encode_position_fn, stage, epoch, model_coarse, model_fine, datafile_idx=None):
    ...
    with torch.no_grad():
        ...
        if datafile_idx: # only validation during weak learning uses datafile_idx, I want to save that into a separate folder
            save_path = os.path.join(save_path, f"val_stage{stage}")
            if not os.path.exists(save_path):
                os.makedirs(save_path)

        # SAVE validation prediction on file #
        cv2.imwrite(f'{save_path}/s{stage}_e{epoch}_ensVal_rgb_coarse.png', cv2.cvtColor(rgb_coarse[..., :3].detach().cpu().numpy() * 255, cv2.COLOR_BGR2RGB))
        cv2.imwrite(f'{save_path}/s{stage}_e{epoch}_ensVal_disp_coarse.png', disp_coarse.detach().cpu().numpy() * 255)
        # cv2.imwrite(f'{save_path}/s{stage}_e{epoch}_ensVal_acc_coarse.png', acc_coarse.detach().cpu().numpy() * 255)
        if rgb_fine is not None:
            cv2.imwrite(f'{save_path}/s{stage}_e{epoch}_ensVal_rgb_fine.png', cv2.cvtColor(rgb_fine[..., :3].detach().cpu().numpy() * 255, cv2.COLOR_BGR2RGB))
            cv2.imwrite(f'{save_path}/s{stage}_e{epoch}_ensVal_disp_fine.png', disp_fine.detach().cpu().numpy() * 255)
            # cv2.imwrite(f'{save_path}/s{stage}_e{epoch}_ensVal_acc_fine.png', acc_fine.detach().cpu().numpy() * 255)
            cv2.imwrite(f'{save_path}/s{stage}_e{epoch}_ensVal_grad.png', cv2.cvtColor((target_ray_values[..., :3].detach() - rgb_fine[..., :3].detach()).cpu().numpy() * 255, cv2.COLOR_BGR2RGB))
        else:
            cv2.imwrite(f'{save_path}/s{stage}_e{epoch}_ensVal_grad.png', cv2.cvtColor((target_ray_values[..., :3].detach() - rgb_coarse[..., :3].detach()).cpu().numpy() * 255, cv2.COLOR_BGR2RGB))
        
        cv2.imwrite(f'{save_path}/s{stage}_e{epoch}_ensVal_target_rgb.png', cv2.cvtColor(target_ray_values[..., :3].detach().cpu().numpy() * 255, cv2.COLOR_BGR2RGB))
